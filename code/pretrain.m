...
filters_name = strcat('depth',int2str(params.depth),'_filters.mat');
patches_name = strcat('depth',int2str(params.depth),'_patches.mat');
params.patchOverlap = 0.01;
showFilters = false;

if(exist(patches_name,'file')==0)
    disp 'patches file not found, will create one'
    ...
    save(patches_name,'patches','P','M');
else
    load(patches_name);
    disp 'Loading existing patches file'
end
if(exist(filters_name,'file'))
    disp 'Loading filters file'
    load(filters_name);
else
    disp 'Computing filters'
    filters = run_kmeans(patches,params.numFilters,100);
    save(filters_name,'filters');
end
